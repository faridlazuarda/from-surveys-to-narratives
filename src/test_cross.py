...
import torch
from .get_translated_prompt import getTranslatedPrompt
import numpy as np
import random
...
def getResponse(prompt, country, context, lora_path, base_model=None, base_tokenizer=None, eval_type=None, adapter_lang=None):
    ...
    # After generation, delete input_ids to free memory
    del input_ids
...
def getPrompt(input_txt, task, adapter_lang=None, eval_type=None):
    if eval_type == "translate" and adapter_lang:
        ...
        elif task in ['stereotype_detect', 'mockery_detect', 'insult_detect', 'improper_detect', 'aggressiveness_detect', 'toxicity_detect', 'negative_stance_detect', 'homophobia_detect', 'racism_detect', 'misogyny_detect', 'threat_detect', 'hostility_directness_detect']:
            entity = task[:-7]
...
def run(
    ...
    print(adapter_lang, test_lang, task, context, model_name, model_type, eval_type)
    
    set_seed()

    # Define the directory path
    dir_path = f'{output_dir}/{model_name.split("/")[-1]}/{adapter_lang.lower()}/{test_lang.lower()}'
    if debug:
        dir_path = f'{output_dir}/{model_name.split("/")[-1]}/debug/{adapter_lang.lower()}/{test_lang.lower()}'
    ...
    result_file = f'{dir_path}/{task}_res{suffix}.jsonl'
    prediction_file = f'{dir_path}/{task}_predictions{suffix}.jsonl'

    # Check if files already exist
    if os.path.exists(result_file) and os.path.exists(prediction_file):
        print(f"Experiment already completed for task {task}. Skipping execution.")
        return
    
    print(f"Starting experiment for task {task}.")
    ...
    pretrained_model, base_tokenizer = get_model_and_tokenizer("combined" if adapter_lang.lower() == "combined" else adapter_lang.lower(), model_name)

    print("test_languages: ", languages)
    ...
    with jsonlines.open(prediction_file, mode='a') as writer:
        for result in results_list:
            writer.write(result)

    print(f"Experiment completed for task {task}. Results saved in {dir_path}.")
