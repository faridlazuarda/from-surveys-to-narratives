...
#SBATCH --job-name=llamainst_no               # Job name
#SBATCH --output=tud/slurm-outputs/output_%A.txt
#SBATCH --nodes=1
#SBATCH --ntasks=1                        # Run on a single CPU
#SBATCH --mem=100G                         # Total RAM to be used
...
train_model() {
    ...
    if [ -d "${out_path}" ]; then
        echo "Training already completed for: ${model_key} - ${training_scenario} - ${culture}. Skipping..."
        return
    fi
    ...
    # Execute Training
    CUDA_VISIBLE_DEVICES=0 python -m src.llama_finetune \
    ...
    for adapter_lang in "${cultures[@]}"; do
        ...
        for test_lang in "${test_languages[@]}"; do
            ...
            for task in ${combined_tasks[$test_lang]}; do
                ...
                # Execute Evaluation
                CUDA_VISIBLE_DEVICES=2 python -m src.test_cross \
