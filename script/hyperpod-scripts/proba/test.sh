...
# Define evaluation scenarios and languages
...
BASE_OUTPUT_DIR="/tud/value-adapters/culturellm/eval_outputs/proba"

# Main loop over scenarios, models, and languages
        ...
        if [[ "$eval_type" == "zero-shot" ]]; then
            ...
            for test_lang in "${test_languages[@]}"; do
                for task in ${combined_tasks[$test_lang]}; do
                    echo "Running zero-shot evaluation for Model: $MODEL_NAME, Test Culture: $test_lang, Task: $task."
            ...
            # Finetuning scenarios: baseline or cultural_context
            for adapter_lang in "${adapter_languages[@]}"; do
                for test_lang in "${test_languages[@]}"; do
                    for task in ${combined_tasks[$test_lang]}; do
                        ADAPTER_PATH="${BASE_MODEL_DIR}/${eval_type}/${MODEL_TYPE}/${adapter_lang}"

                        echo "Running ${eval_type} evaluation for Model: $MODEL_NAME, Adapter: $adapter_lang, Test Culture: $test_lang, Task: $task."
