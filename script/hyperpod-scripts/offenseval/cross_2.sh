...
#SBATCH --job-name="cross2"
#SBATCH --output="/tud/outputs/run_test_cross2.out"
#SBATCH --gres=gpu:1
#SBATCH -N 1 
#SBATCH -A nlp-dept 
#SBATCH -p nlp-dept 
...
adapter_languages=("greek" "korean" "portuguese" "spanish" "turkish")
    ...
    for test_lang in "${test_languages[@]}"; do
        if [ "$adapter_lang" != "$test_lang" ]; then
            for task in ${combined_tasks[$test_lang]}; do
                ...
                CUDA_VISIBLE_DEVICES=$VISIBLE_DEVICE python -m src.test_cross \
